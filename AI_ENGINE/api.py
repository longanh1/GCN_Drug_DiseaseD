...
# Inline variant metadata — avoids importing torch at API startup
def _ablation_configs() -> dict:
    return {
        'sim_only':            dict(name_vi='Chỉ dùng Similarity',                       name_en='Similarity Only',                      group='similarity_only', color='#3B82F6', icon='📐'),
        'gcn_only':            dict(name_vi='Chỉ dùng GCN (Heterogeneous Network)',       name_en='Network (GCN) Only',                   group='network_only',    color='#10B981', icon='🕸️'),
        'sim_transformer':     dict(name_vi='Similarity + Transformer',                   name_en='Similarity Feature Extraction',         group='similarity_fe',   color='#8B5CF6', icon='🧬'),
        'gcn_transformer':     dict(name_vi='Heterogeneous Graph + Transformer nâng cao', name_en='Network Feature Extraction (Advanced)', group='network_fe',      color='#F59E0B', icon='🔬'),
        'sim_gcn':             dict(name_vi='Fusion: Similarity + GCN',                   name_en='Fusion (Sim + GCN)',                    group='fusion',          color='#EF4444', icon='🔗'),
        'sim_transformer_gcn': dict(name_vi='Fusion: Similarity + Transformer + GCN',    name_en='Fusion (Sim + Transformer + GCN)',      group='fusion',          color='#F97316', icon='⚡'),
        'full':                dict(name_vi='Full Model: Similarity + GCN + Modality Interaction', name_en='Full Model (AMNTDDA_Fuzzy)',   group='full_model',      color='#6366F1', icon='🎯'),
    }
...
def get_ablation_comparison(dataset: str = Query("C-dataset")):
    """Return master ablation comparison JSON for all 7 pipeline variants."""
    results_dir = os.path.join(DATA_OUT_DIR, 'results')
    cp = os.path.join(results_dir, f'{dataset}_ablation_comparison.json')
    if os.path.exists(cp):
        with open(cp, encoding='utf-8') as fh:
            return json.load(fh)
    # No training done yet — return metadata skeleton so UI can show instructions
...
@app.get("/results/ablation/variant")
def get_ablation_variant(dataset: str = Query("C-dataset"),
                          variant: str = Query("full")):
    """Return summary + per-fold results for one ablation variant."""
    results_dir = os.path.join(DATA_OUT_DIR, 'results')
    metrics_keys = ['AUC', 'AUPR', 'Accuracy', 'Precision', 'Recall', 'F1', 'MCC']

    summary = None
    sp = os.path.join(results_dir, f'{dataset}_ablation_{variant}_summary.json')
    if os.path.exists(sp):
        with open(sp, encoding='utf-8') as fh:
            summary = json.load(fh)

    folds: list = []
    cp = os.path.join(results_dir, f'{dataset}_ablation_{variant}_fold_results.csv')
    if os.path.exists(cp):
        df = pd.read_csv(cp)
        df_f = df[pd.to_numeric(df['fold'], errors='coerce').notna()]
        folds = df_f.to_dict(orient='records')
...
@app.get("/results/ablation/all_variants")
def get_all_ablation_variants(dataset: str = Query("C-dataset")):
    """Return summary for each variant (no fold details) for quick comparison."""
    results_dir = os.path.join(DATA_OUT_DIR, 'results')
    metrics_keys = ['AUC', 'AUPR', 'Accuracy', 'Precision', 'Recall', 'F1', 'MCC']
    configs  = _ablation_configs()
    output   = {}
    for variant in ABLATION_VARIANT_ORDER:
        ...
        if os.path.exists(sp):
            ...
            output[variant] = {
                'variant': variant,
                'trained': False,
                **{f'{m}_mean': None for m in metrics_keys},
                **{f'{m}_std':  None for m in metrics_keys},
                **cfg,
            }
    return {
        "dataset": dataset,
