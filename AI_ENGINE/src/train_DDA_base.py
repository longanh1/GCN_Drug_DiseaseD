"""
train_DDA_base.py — Chạy model AMNTDDA gốc (không có Fuzzy Logic)
và lưu kết quả vào AI_ENGINE/data/results/ để hiển thị trên web.

Không sửa file nào trong AMDGT_main — áp dụng compatibility patches
cho NetworkX 3.x và DGL mới trong bộ nhớ trước khi import.

Usage:
    python train_DDA_base.py --dataset C-dataset
"""
...
# ── Compatibility patches (KHÔNG sửa file gốc) ───────────────────────
...
def update_comparison(dataset: str):
    """Gộp summary của AMNTDDA và AMNTDDA_Fuzzy vào comparison.json."""
    comparison = {'dataset': dataset}
    for model_name in (MODEL_NAME, 'AMNTDDA_Fuzzy'):
...
if __name__ == '__main__':
    ...
    for i in range(args.k_fold):
        ...
        best = dict(auc=0, aupr=0, accuracy=0,
                    precision=0, recall=0, f1=0, mcc=0)
