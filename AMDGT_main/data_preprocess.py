...
def dgl_heterograph(data, drdi, args):
    ...
    heterograph_dict = {
        ('drug', 'association', 'disease'): tuple(zip(*drdi_list)) if drdi_list else ([], []),
        ('drug', 'association', 'protein'): tuple(zip(*drpr_list)) if drpr_list else ([], []),
        ('disease', 'association', 'protein'): tuple(zip(*dipr_list)) if dipr_list else ([], [])
    }
    ...
    drdipr_graph = dgl.heterograph(heterograph_dict, num_nodes_dict=node_dict)  # type: ignore[operator]
