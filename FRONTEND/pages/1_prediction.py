        ...
        if not trained_keys:
            ...
            with at3:
                ...
                <div style="background:linear-gradient(135deg,{meta['color']}18,{meta['color']}06);
                            border-left:4px solid {meta['color']};border-radius:12px;
                            padding:18px 20px;margin-bottom:20px;">
                    <div style="font-size:1.4rem;font-weight:800;color:{meta['color']};">
                        {meta['icon']}&nbsp;&nbsp;{meta['label']}
                    </div>
                    <div style="color:#64748b;font-size:0.85rem;margin-top:6px;line-height:1.5;">
                        {d.get('desc', meta.get('desc',''))}
                    </div>
                ...
                cfg_items = [
                    ("Similarity Graph (GT)", d.get("use_sim", False), "#6366f1"),
                    ("Network Graph (HGT)",   d.get("use_gcn", False), "#10b981"),
                    ("Transformer",           d.get("use_trans", False), "#f59e0b"),
                    ("Cross-Modal Attention", d.get("cross_modal", False), "#ef4444"),
                ]
                cfg_html = '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px;">'
                for label, active, color in cfg_items:
                    if active:
                        cfg_html += f'<div style="background:{color}22;border:1.5px solid {color};color:{color};padding:6px 14px;border-radius:8px;font-size:0.82rem;font-weight:700;">✅ {label}</div>'
                    else:
                        cfg_html += f'<div style="background:#f1f5f9;border:1.5px solid #cbd5e1;color:#94a3b8;padding:6px 14px;border-radius:8px;font-size:0.82rem;">❌ {label}</div>'
                cfg_html += '</div>'
                ...
                METRIC_META = {
                    "AUC":       ("🎯 AUC",       "#6366f1", "Diện tích dưới đường ROC — độ phân biệt tổng thể"),
                    "AUPR":      ("📈 AUPR",      "#8b5cf6", "Diện tích dưới đường PR — quan trọng khi dữ liệu mất cân bằng"),
                    "Accuracy":  ("✅ Accuracy",  "#10b981", "Tỷ lệ dự đoán đúng tổng thể"),
                    "Precision": ("🎯 Precision", "#f59e0b", "Độ chính xác của dự đoán dương tính"),
                    "Recall":    ("🔍 Recall",    "#ef4444", "Khả năng tìm đúng các liên kết thực"),
                    "F1":        ("⚖️ F1-Score",  "#06b6d4", "Cân bằng giữa Precision và Recall"),
                    "MCC":       ("🧮 MCC",       "#84cc16", "Matthews Correlation — chỉ số cân bằng nhất"),
                }

                # Hàng card lớn cho 3 chỉ số chính
                main_metrics = ["AUC", "AUPR", "F1"]
                cols_main = st.columns(3)
                for ci, m in enumerate(main_metrics):
                    mv = d.get(f"{m}_mean")
                    sv = d.get(f"{m}_std")
                    mlabel, mcolor, mdesc = METRIC_META[m]
                    pct = int((mv or 0) * 100)
                    with cols_main[ci]:
                        st.markdown(f"""
                        <div style="background:linear-gradient(135deg,{mcolor}14,{mcolor}05);
                                    border:1.5px solid {mcolor}50;border-radius:12px;
                                    padding:16px;text-align:center;">
                            <div style="font-size:0.82rem;color:#64748b;margin-bottom:6px;">{mlabel}</div>
                            <div style="font-size:2rem;font-weight:800;color:{mcolor};">
                                {f"{mv:.4f}" if mv is not None else "—"}
                            </div>
                            <div style="font-size:0.75rem;color:#94a3b8;margin-top:2px;">
                                ± {f"{sv:.4f}" if sv is not None else "—"}
                            </div>
                            <div style="background:#e2e8f0;border-radius:20px;height:6px;margin-top:10px;overflow:hidden;">
                                <div style="background:{mcolor};height:100%;width:{pct}%;border-radius:20px;"></div>
                            </div>
                            <div style="font-size:0.7rem;color:#94a3b8;margin-top:4px;">{mdesc}</div>
                        </div>""", unsafe_allow_html=True)

                st.markdown("<br>", unsafe_allow_html=True)

                # Bảng mở rộng cho tất cả 7 chỉ số
                with st.expander("📋 Xem đầy đủ 7 chỉ số", expanded=True):
                    rows_detail = []
                    for m in METRICS:
                        mv = d.get(f"{m}_mean")
                        sv = d.get(f"{m}_std")
                        ...
                        rows_detail.append({
                            "Chỉ số": mlabel,
                            "Giá trị (mean)": f"{mv:.6f}" if mv is not None else "—",
                            "Độ lệch (±std)": f"{sv:.6f}" if sv is not None else "—",
                            "Mô tả": mdesc,
                        })
                    import pandas as _pd_det
                    df_detail = _pd_det.DataFrame(rows_detail).set_index("Chỉ số")
                    st.dataframe(
                        df_detail,
                        use_container_width=True,
                        column_config={
                            "Giá trị (mean)": st.column_config.TextColumn(width="medium"),
                            "Độ lệch (±std)": st.column_config.TextColumn(width="medium"),
                            "Mô tả":          st.column_config.TextColumn(width="large"),
                        },
                    )

                # So sánh nhanh với các biến thể khác
                if len(trained_keys) > 1:
                    st.markdown("##### 🏆 Xếp hạng so với các biến thể khác")
                    rank_cols = st.columns(len(["AUC","F1","MCC"]))
                    for ci, m in enumerate(["AUC", "F1", "MCC"]):
                        scores = sorted(
                            [(k, trained_data[k].get(f"{m}_mean") or 0) for k in trained_keys],
                            key=lambda x: x[1], reverse=True
                        )
                        rank = next((i+1 for i, (k,_) in enumerate(scores) if k == sel_k), "?")
                        best_v = scores[0][1]
                        cur_v  = d.get(f"{m}_mean") or 0
                        diff   = cur_v - best_v
                        _, mcolor, _ = METRIC_META[m]
                        medal = "🥇" if rank == 1 else ("🥈" if rank == 2 else ("🥉" if rank == 3 else f"#{rank}"))
                        with rank_cols[ci]:
                            st.markdown(f"""
                            <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;
                                        padding:12px;text-align:center;">
                                <div style="font-size:0.78rem;color:#64748b;">{m}</div>
                                <div style="font-size:1.6rem;">{medal}</div>
                                <div style="font-size:1.1rem;font-weight:700;color:{mcolor};">{cur_v:.4f}</div>
                                <div style="font-size:0.75rem;color:{'#10b981' if diff >= 0 else '#ef4444'};">
                                    {'=' if diff == 0 else (f'+{diff:.4f}' if diff > 0 else f'{diff:.4f}')} vs best
                                </div>
                            </div>""", unsafe_allow_html=True)
