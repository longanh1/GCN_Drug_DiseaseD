...
#
# REQUIREMENTS:
#   - SQLite (auto-created as pharmalink.sqlite on first start)
#   - No PostgreSQL/pgAdmin 4 required
...
Write-Host "Database: SQLite (pharmalink.sqlite -- auto-created)" -ForegroundColor DarkCyan
...
$cmd1 = "cd `"$root\AI_ENGINE`"; & `"$python`" api.py"
...
$cmd2 = "cd `"$root\BACKEND`"; npm run start:dev"
...
$cmd3 = "cd `"$root\FRONTEND`"; & `"$streamlit`" run app.py"
